<x-admin-layout>
      ...
      <div class="row align-items-center">
        <div class="col-md-12">
          ...
          <ul class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="#!">{{__('admin.EditDiscount')}}</a></li>
          </ul>
        </div>
      </div>
    </div>
  ...
  <div class="row">
    <div class="col-sm-12">
      <form action="{{url('admin/discounts/update')}}" method="POST" id="discountForm" novalidate>
        @csrf
        <div class="card">
          <div class="card-header">
            <h5>{{__('admin.EditDiscount')}}</h5>
          </div>
          <div class="card-body">
            <div class="row">
              <div class="col col-8">
                <div class="form-group">
                  ...
                  <input type="text" name="code" value="{{$discount->code}}" class="form-control" id="inputCode" autocomplete="code" autofocus placeholder="{{__('admin.CouponCode')}}">
                  <div class="d-block"></div>
                </div>
                <div class="form-group">
                  <label for="inputName">{{__('admin.CouponName')}}</label>
                  <input type="text" name="name" value="{{$discount->name}}" class="form-control" id="inputName" placeholder="{{__('admin.CouponName')}}">
                </div>
                <div class="form-group">
                  <label for="inputDescription">{{__('admin.Description')}}</label>
                  <textarea name="description" id="inputDescription" class="form-control">{{$discount->description}}</textarea>
                </div>
                <div class="row">
                  <div class="form-group col-md-6">
                    <label for="inputStartsAt">{{__('admin.StartsAt')}}</label>
                    <input type="date" name="starts_at" value="{{ \Carbon\Carbon::parse($discount->starts_at)->format('Y-m-d') }}" class="form-control" id="inputStartsAt" autocomplete="off">
                    <div class="d-block"></div>
                  </div>
                  <div class="form-group col-md-6">
                    <label for="inputExpiresAt">{{__('admin.ExpiresAt')}}</label>
                    <input type="date" name="expires_at" value="{{ \Carbon\Carbon::parse($discount->expires_at)->format('Y-m-d')}}" class="form-control" id="inputExpiresAt" autocomplete="off">
                    <div class="d-block"></div>
                  </div>
                </div>
              </div>
              <div class="col col-4">
                <div class="form-group mt-4">
                  <div class="custom-control custom-checkbox">
                    <input type="checkbox" name="status" @checked($discount->status) class="custom-control-input" id="inputStatus">
                ...
                <div class="form-group">
                  ...
                  <select name="type" class="form-control" id="inputType">
                    <option value="fixed" @selected($discount->type == 'fixed')>{{__('admin.Fixed')}}</option>
                    <option value="percent" @selected($discount->type == 'percent')>{{__('admin.Percent')}}</option>
                  </select>
                </div>
                <div class="form-group">
                  <label for="inputMaxUses">{{__('admin.MaxUses')}}</label>
                  <input type="number" name="max_uses" value="{{$discount->max_uses}}" class="form-control" id="inputMaxUses">
                </div>
                <div class="form-group">
                  <label for="inputMaxUsesUser">{{__('admin.MaxUsesUser')}}</label>
                  <input type="number" name="max_uses_user" value="{{$discount->max_uses_user}}" class="form-control" id="inputMaxUsesUser">
                </div>
                <div class="form-group">
                  <label for="inputDiscountAmount">{{__('admin.DiscountAmount')}}</label>
                  <input type="number" name="discount_amount" value="{{$discount->discount_amount}}" class="form-control" id="inputDiscountAmount">
                  <div class="d-block"></div>
                </div>
                <div class="form-group">
                  <label for="inputMinAmount">{{__('admin.MinAmount')}}</label>
                  <input type="number" name="min_amount" value="{{$discount->min_amount}}" class="form-control" id="inputMinAmount">
                </div>
              </div>
            </div>
          </div>
  ...
  <script>
    ...
    discountForm.addEventListener('submit', function(event) {
      ...
      fetch('/admin/discounts/{{$discount->id}}', {
        method: 'PUT',
        headers: {
