<x-admin-layout>
  <x-slot:title>@lang('admin.EditPartner')</x-slot:title>
  <div class="page-header">
    <ul class="breadcrumb">
      <li class="breadcrumb-item"><a href="/admin"><i class="feather icon-home"></i></a></li>
      <li class="breadcrumb-item"><a href="/admin/partners">@lang('admin.Partners')</a></li>
      <li class="breadcrumb-item"><a href="#!">@lang('admin.EditPartner')</a></li>
    </ul>
  </div>
  <form action="{{url('admin/partners/' . $partner->id)}}" method="POST" id="partnerForm" novalidate>
    @csrf
    @method('PUT')
    <div class="card">
      <div class="card-header">
        <h5>{{__('admin.EditPartner')}}</h5>
      </div>
      <div class="card-body">
        ...
        <div class="row">
          <div class="col col-md-8">
            <div class="form-group">
              ...
              <input type="text" name="company" value="{{$partner->company}}" class="form-control" id="inputCompany" placeholder="@lang('admin.Company')" autofocus>
              <div class="invalid-feedback d-block"></div>
            </div>
            <div class="form-group">
              <label for="inputName">{{__('admin.Name')}}</label>
              <input type="text" name="name" value="{{$partner->name}}" class="form-control" id="inputName" placeholder="{{__('admin.InsertName')}}">
              <div class="invalid-feedback d-block"></div>
            </div>
            <div class="form-group">
              <label for="inputSurname">@lang('admin.Surname')</label>
              <input type="text" name="surname" value="{{$partner->surname}}" class="form-control" id="inputSurname" placeholder="@lang('admin.Surname')">
              <div class="invalid-feedback d-block"></div>
            </div>
            <div class="form-group">
              <label for="inputEmail">Email</label>
              <input type="email" name="email" value="{{$partner->email}}" class="form-control" id="inputEmail" placeholder="Email">
              <div class="invalid-feedback d-block"></div>
            </div>
            <div class="form-group">
              <label for="inputPhone">@lang('admin.Phone')</label>
              <input type="tel" name="phone" value="{{$partner->phone}}" class="form-control" id="inputPhone" placeholder="@lang('admin.Phone')">
              <div class="invalid-feedback d-block"></div>
            </div>
            <div class="form-group">
              <label for="inputCity">@lang('admin.City')</label>
              <input type="text" name="city" value="{{$partner->city}}" class="form-control" id="inputCity" placeholder="@lang('admin.City')">
              <div class="invalid-feedback d-block"></div>
            </div>
            <div class="form-group">
              <label for="inputAddress">@lang('admin.Address')</label>
              <input type="text" name="address" value="{{$partner->address}}" class="form-control" id="inputAddress" placeholder="@lang('admin.Address')">
              <div class="invalid-feedback d-block"></div>
            </div>
            <div class="form-group">
              <label for="inputCoordinates">@lang('admin.Coordinates')</label>
              <input type="text" name="coordinates" value="{{$partner->coordinates}}" class="form-control" id="inputCoordinates" placeholder="@lang('admin.Coordinates')">
              <div class="invalid-feedback d-block"></div>
            </div>
            <div class="form-group">
              <label for="inputSite">@lang('admin.Site')</label>
              <input type="text" name="site" value="{{$partner->site}}" class="form-control" id="inputSite" placeholder="@lang('admin.Site')">
              <div class="invalid-feedback d-block"></div>
            </div>
            <div class="form-group">
              <label for="inputSchedule">@lang('admin.Schedule')</label>
              <input type="text" name="schedule" value="{{$partner->schedule}}" class="form-control" id="inputSchedule" placeholder="@lang('admin.Schedule')">
              <div class="invalid-feedback d-block"></div>
            </div>
          </div>
          <div class="col col-md-4">
            <div class="form-group">
              ...
              <input type="number" name="order_by" value="{{$partner->order_by}}" class="form-control" id="inputOrderBy">
            </div>
            <div class="form-group">
              <div class="custom-control custom-checkbox">
                <input type="checkbox" name="status" @checked($partner->status) class="custom-control-input" id="inputStatus">
            ...
            <div class="form-group">
              <div class="upload-preview">
                ...
                <div class="position-relative mb-3">
                  <img class="w-100" src="{{asset('uploads/partners/' . $partner->picture)}}">
    ...
    <div class="fixed-bottom pt-3 pb-3 w-100 bg-light" style="padding-left: 260px; z-index: 10; box-shadow: -1px 0 2px lightgrey;">
      ...
      <button type="button" data-url="{{ url('admin/partners') }}" onclick="if(!confirm('Вы уверены, что хотите отменить изменения?')) {return false} else {window.location.href = this.dataset.url}" class="ml-3 btn btn-outline-warning">{{__('admin.Cancel')}}</button>
    </div>
  </form>
  @push('scripts')
  <script>
    ...
    function removePicture() {
      fetch('/admin/partners/{{$partner->id}}/remove-picture').then(response => response.json()).then(data => {
