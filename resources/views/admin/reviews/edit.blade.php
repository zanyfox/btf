<x-admin-layout>
      ...
      <div class="row align-items-center">
        <div class="col-md-12">
          ...
          <ul class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="#!">{{__('admin.EditReview')}}</a></li>
          </ul>
        </div>
      </div>
    </div>
  ...
  <div class="row">
    <div class="col-sm-12">
      <form action="{{route('admin.reviews.update', $review->id)}}" method="POST" enctype="multipart/form-data" novalidate>
        @csrf
        @method('PUT')
        <div class="card">
          <div class="card-header">
            <h5>{{__('admin.EditReview')}}</h5>
          </div>
          <div class="card-body">
            <div class="row">
              <div class="col col-8">
                <div class="form-group">
                  ...
                  <input type="text" name="name" class="form-control" value="{{old('name') ?? $review->name}}" id="inputName" autofocus>
                  @error('name')<div class="invalid-feedback d-block">{{$message}}</div>@enderror
                </div>
                <div class="form-group">
                  <label for="inputContent">{{__('admin.Text')}}</label>
                  <textarea id="inputContent" name="content" rows="5" class="form-control">{{old('content') ?? $review->content}}</textarea>
                </div>
              </div>
              <div class="col col-4">
                <div class="form-group mt-4">
                  <div class="custom-control custom-checkbox">
                    <input type="checkbox" name="status" @checked($review->status) class="custom-control-input" id="inputStatus">
                ...
                <div class="fileUpload">
                  ...
                  <div class="text-center mb-2">
                    <img src="{{asset('uploads/reviews/' . $review->picture)}}" class="hei-100 wid-100 bg-light d-inline-block border border-secondary rounded-circle" width="70" alt="">
                  </div>
                  @endif
