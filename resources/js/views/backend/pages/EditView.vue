<template>
  ...
  <form @submit.prevent="updatePage($route.params.id)">
    <div class="card pb-5">
            ...
            <div class="row">
              <div class="col col-8">
                <div class="mb-3">
                  ...
                  <input type="text" v-model.trim="page.name" class="form-control" id="inputName" autocomplete="name">
                  <div v-if="errors.name" class="invalid-feedback d-block">{{ errors.name[0] }}</div>
                </div>
                <div class="mb-3">
                  <label for="inputSlug" class="form-label">Слаг</label>
                  <input type="text" v-model.trim="form.slug" class="form-control" id="inputSlug">
                  <div v-if="errors.slug" class="invalid-feedback d-block">{{ errors.slug[0] }}</div>
                </div>
                <div class="mb-3">
...
const { page, getPage, updatePage, errors } = usePages()
...
onMounted(() => {
  getColors()
  //getPage(route.params.id).then(() => {
  getPage(props.id).then(() => {
    form.name = page.value.name
    form.slug = page.value.slug
    form.status = page.value.status === 1
  })
})
</script>
